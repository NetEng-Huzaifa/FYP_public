...
def ip_checking(user_ip):
    ...
    if len(user_ip) == 4:  # check no. of octats of IP
        if int(user_ip[3]) == 0:  # check last octat of IP
            # for Network address
            mgbx.showinfo("Selection", "It is a Network address")
        else:
        ...
        mgbx.showinfo("Selection", f"""Please Check your entered IPv4 Address\nno. of Valid Octat of IPv4 is 4 but you entered {len(user_ip)}""")
...
def selection_device(vendor, device):
    if vendor == "Cisco":
        ...
        mgbx.showinfo("Selection", f"Please select given {vendor} devices")


def telnet_device(ip, user, pwd):
    try:
        telnet = Telnet(ip)
        # telnet.open(ip)
